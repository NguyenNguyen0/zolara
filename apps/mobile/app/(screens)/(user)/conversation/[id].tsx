import ConversationHeader from '@/src/components/ui/conversation.header';
import TimeStamp from '@/src/components/ui/divider.timestamp';
...
import { MOCK_GROUP_CHAT, MOCK_FRIEND_CHAT, MOCK_STRANGER_CHAT } from '@/src/mocks/conversation';
import {
	getHeaderStatusFromConversationType,
	getOnlineStatusText,
	formatTimestamp,
} from '@/src/utils/convertHelper';
...
export default function ConversationDetail() {
	...
	const [conversation] = useState<Conversation>(MOCK_STRANGER_CHAT);

	const isGroupConversation = conversation.type === 'GROUP';

	// Check if timestamp separator should be shown (every 30 minutes)
	const shouldShowTimestamp = (currentMsg: Message, prevMsg?: Message): boolean => {
	...
	const shouldShowAvatar = (currentMsg: Message, prevMsg?: Message): boolean => {
		if (!prevMsg) return true;
	...
	const shouldShowTime = (currentMsg: Message, nextMsg?: Message): boolean => {
	...
	const renderItem = ({ item, index }: { item: Message; index: number }) => {
		...
		const prevMsg = index < reversedMessages.length - 1 ? reversedMessages[index + 1] : undefined; // Prev in display = next in reversed array
		...
		const user = conversation.members.find((m) => m.id === item.userId) || conversation.members[0];
		...
		return (
			<>
				<MessageItem
					...
					showTime={showTime}
				/>
				{showTimestamp && <TimeStamp time={formatTimestamp(item.timestamp)} />}
			</>
		);
	};

	// Map conversation type to header status
	const getHeaderStatus = (): 'stranger' | 'friend' | 'friend_request' => {
	...
	return (
		<SafeAreaView edges={['top', 'bottom']} style={{ flex: 1 }}>
			<ConversationHeader
				...
				onAcceptFriendRequest={() => console.log('Accept friend request')}
			/>
